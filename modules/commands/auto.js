  ...
  for (const el of arr) {
    ...
    if (regEx_instagram.test(el)) {
      try {
        ...
        const res = await axios({
          url: `https://apidown.site/api/instagram/v1?link=${encodeURIComponent(el)}`,
          method: 'GET'
        });
    ...
    // Auto SoundCloud
    if (regEx_SC.test(el)) {
      const audioStream = await axios.get(`https://apidown.site/api/soundcloud/v1?link=${encodeURIComponent(el)}`, {
        responseType: 'stream'
      });
    ...
    if (regEx_ZingMp3.test(el)) {
      const audioStream = await axios.get(`https://apidown.site/api/zingmp3/v1?link=${encodeURIComponent(el)}`, {
        responseType: 'stream'
      });
      audioStream.data.path = 'sing.mp3';
      return out({
