  ...
  return async (event) => {
    ...
    let find_thuebot = thuebot.find($ => $.t_id == event.threadID);
       if (((global.data.threadData.get(event.threadID)?.PREFIX || global.config.PREFIX) + 'bank') != event.args[0]) {
          if (!find_thuebot) return api.shareContact(`❎ Nhóm của bạn chưa thuê bot, vui lòng liên hệ Admin Sakura để thuê bot\n🌐 Facebook: ${config.FACEBOOK_ADMIN}\n🔐 Zalo: ${config.ZALO_ADMIN}`, global.config.NDH[0], event.threadID);
          if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) return api.shareContact(`⚠️ Nhóm của bạn đã hết hạn thuê bot, vui lòng thanh toán để tiếp tục gia hạn, liên hệ Admin Sakura\n🌐 Facebook: ${config.FB_ADMIN}\n🔐 Zalo: ${config.ZALO_ADMIN}`, global.config.NDH[0], event.threadID);
       };
    }
